...
```{r}
#first full model
ols_model <- lm(data$MORT ~., data = data)
summary(ols_model)
```
### Mallow's $C_p$

```{r}
#subset the modeling using Mallow's Cp
library(MASS)
...
# Step 1: All Subsets Regression and Cp Calculation
all_subsets <- regsubsets(MORT  ~ ., data = data, nvmax = length(predictors))
subsets_summary <- summary(all_subsets)

# Extract Mallows' Cp and number of predictors
cp_data <- data.frame(
  num_predictors = 1:length(subsets_summary$cp),
  Cp = subsets_summary$cp
)

# Step 2: Visualize Mallows' Cp with ggplot2
ggplot(cp_data, aes(x = num_predictors, y = Cp)) +
  geom_point() +
  geom_line() +
  geom_point(data = cp_data[which.min(cp_data$Cp), ], aes(x = num_predictors, y = Cp),
             color = "red", size = 3) +
  labs(
    title = "Mallows' Cp Plot for McDonald Data",
