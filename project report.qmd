...
predictors <- setdiff(names(data.norm),response)
...
cp_dat <- data.frame(num_pred = 1:length(subsets_summary$cp), cp = subsets_summary$cp)

#Step 3: use ggplot to visualize this
ggplot(cp_dat, aes(x = num_pred, y = cp)) +
  geom_point() +
  geom_line() +
  geom_point(data = cp_dat[which.min(cp_dat$cp), ], aes(x = num_pred, y = cp),
             color = "red", size = 3) +
  labs(
...
min_cp_index <- which.min(subsets_summary$cp)
