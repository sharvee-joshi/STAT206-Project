...
# compare the AIC, ridge regression, CP Mallow, and PCA to the full model 
comparison <- data.frame(
  Model = c("OLS Full", "Ridge", "Mallows' C_p", "PCR"),
  R2 = c(0.7648, 0.545, 0.7648, 0.712),
  AIC = c(AIC(ols_model), AIC(ridge_model), AIC(red_cp_model), NA)
)
kable(comparison, caption = "Model Comparison")
```

From these comparisons, the Mallows’ Cp model emerged as the best-performing model, achieving the lowest AIC value and the highest R-squared. Ridge Regression also performed well in terms of AIC, but its slightly higher MSE suggests it is less effective in minimizing prediction errors. The PCR model, while innovative, did not outperform Mallows’ Cp or Ridge Regression, largely due to its reliance on reduced components that excluded some influential predictors.
...
This project replicated and extended the analyses from the article “Instabilities of Regression Estimates Relating Air Pollution to Mortality”, using modern statistical techniques and computational tools. The original study focused on modeling the relationship between air pollution and mortality, addressing challenges like multicollinearity. While adhering to the original methodology, we introduced Ridge Regression, Mallows’ C_p, and Principal Component Regression (PCR) to extend the analysis and improve computational efficiency.

Our key findings include:
	1.	Ridge Regression effectively stabilized coefficients, particularly for multicollinear variables like NO_x and HC, with improved variance but a slight trade-off in bias. However, the Ridge model’s predictive accuracy, measured through R^2, lagged behind the Mallows’ C_p and OLS models.
	2.	Mallows’ C_p variable selection was implemented with an optimized algorithm, avoiding exhaustive computation of over 32,000 models. This streamlined approach yielded a reduced model with the highest R^2 (0.7648) and the lowest AIC, outperforming other models in terms of prediction accuracy and computational efficiency.
	3.	Principal Component Regression (PCR) introduced a modern perspective, addressing multicollinearity through dimensionality reduction. Although PCR retained predictive accuracy comparable to OLS, its reliance on reduced components led to a marginally lower R^2 (0.712) than the Mallows’ C_p model. Nevertheless, it highlighted alternative ways to stabilize coefficients and simplify models.

Key Contributions and Deviations

Compared to the original study, we:
- Improved computational efficiency by refining Mallows’ C_p selection.
- Enhanced the analysis by introducing PCR, providing a modern extension to address multicollinearity without relying on manual variable selection.
- Incorporated exploratory data analysis, offering insights into data patterns and relationships, which were absent in the original article.

Despite these advancements, challenges such as persistent multicollinearity among pollution variables (HC, NO_x, and SO_x) remain unresolved. The original study’s critique of pollution variables being inherently interdependent was confirmed.
