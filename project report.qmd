...
#print(comparison)
knitr::kable(comparison, theme = "Minimal") 
```
Step 4: Multi-Collinearity Analysis
```{r}
#Find VIF
library(car)
...
knitr::kable(data.frame(Method = c("OLS", "Ridge"), MSE = c(ols_mse, ridge_mse)))
```
...
In the future, we would really like to address the multi-collinearity problem. This seemed to be the biggest problem in the data, and despite running three different kinds of analyses' to address the problem, the multi-collinearity situation was never resolved. If we were to continue with the project, it would be nice to divert our attention on addressing this rather than spending time recreating the article. It would be nice to see if, perhaps, creating a new interaction term between the chemical pollutants, HC, $NO_x$, and $SO_x$ could resolve it. Another interaction term between precipitation and January temperatures could also be created, as there as multi-collinearity between those two as well. By adding these terms into the model and running our tests again with those two terms adding, it would be interesting to see what results we would achieve, and if in fact, the multi-collinearity lessens.
