...
We know that in a multiple regression model, when the predictors ($X_i$s) are linearly dependent (correlated), there is multicollinearity in the model, which causes the following issue for the fitted multiple regression model:
...
We have 15 predictors in our dataset, and we want to investigate the relationship between them and the response variable (mortality). There is a high correlation between most of the predictors. Figure-pcr-1 illustrates this:
...
In PCR, we try to provide a weighted linear combination of the predictors ($X_i$s) like:
$$Z_k = v_{k1}X_1 + v_{k2}X_2 + \dots + v_{kp}X_p$$
We then replace the correlated variables $[X_1, X_2, \dots, X_p]$ with $Z = [Z_1, Z_2, \dots, Z_p]$, which are uncorrelated, so the regression model $y = Z\beta + \epsilon$ does not suffer from multicollinearity.

First, we need to centralize and normalize all the variables for PCR. In the following, we assume the variables are all centralized and normalized and show how to find the $Z$ matrix.


Correlated $X=[X_1, X_2, \dots, X_p]$ $\rightarrow$ Uncorrelated $Z=[Z_1, Z_2, \dots, Z_p]$

We call each $Z_i$ a **principal component**.
...
We perform PCA on matrix $X$ through singular value decomposition. This way we obtain $$X=UDV'$$ 
$D$ is a diagonal matrix consisting of
 non-negative singular values, and $V$ is the matrix of the eigenvectors. we calculate matrix $$Z=XV$$

The columns of $Z$ are linearly independent.
if we define

$$\hat{\beta_z}=\left( Z'Z \right)^{-1}Z'y$$ 
then:

$$\hat{y}=X\hat{\beta_x}=Z\hat{\beta_z}$$
...
colnames(variation) <- c("% of variation","   ","cumulative % of variation")
rownames(variation) <- c("PC1","PC2","PC3","PC4","PC5","PC6","PC7","PC8","PC9","PC10","PC11","PC12","PC13","PC14","PC15")
round(variation,1)
```

The first principal component ($PC_1$) only accounts $27$% of the variation (information) of $X$ matrix, the first two principal components ($PC_1$ and $PC_2$) account $30$% of the $X$ matrix and so on. All the principal components matrix,$Z=[PC_1, PC_2,\dots,PC_{15}]$, account $100$% of the variation of of the $X$ matrix.
...
$$PC_1, PC_3, PC_6, PC_7, PC_9, PC_{12}$$
These subsets of the principal components account for `{r} round(sum(variation[c(1,3,6,7,9,12),1]))`% of the variation in the original predictors ($X_i$s). We may say that the other 49% of the variation is not significantly related to the response variable.
...
$$\hat{y}= 0.251PC_1-0.319PC_3+0.339PC_6 -0.279PC_7+0.3PC_9+0.470PC_{12} $$
We know each PC is a linear combination of all the original predictors, as follow:
...
```{r}
plot(reduced_pcr$fitted.values,reduced_pcr$residuals)
hist(reduced_pcr$residuals,breaks=13)
...
So we can rewrite the reduced model in terms of the original predictors:

$$\hat{\beta_x}=V\hat{\beta_z}$$
where V is the vector of selected eigenvectors corresponding to selected principal components($[PC_1, PC_3, PC_6, PC_7, PC_9, PC_{12}]$)
```{r}
...
\end{align}
$$
For this model $R^2=0.712$ which is 14% larger than the corresponding Ridge model (with k=0.2). The Residual Sum of Squared for this model is 0.287 which is slightly larger than the Residual Sum of Squared for the Ridge model.
