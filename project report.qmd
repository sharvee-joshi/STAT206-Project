...
```
Based off of the residuals plot, we do not see any patterns present. Moving onto the qq-plot, we do some slight curvature towards the end of the line, maybe indicating that the addition of a squared value in the predictors could be useful. The Shapiro Wilks test is satisfied, so our assumptions are met, with some deviation in the linear aspect.
